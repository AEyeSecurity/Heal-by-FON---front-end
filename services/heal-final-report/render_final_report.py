...
def document_xml(report: dict[str, Any], metadata: dict[str, Any], payload: dict[str, Any]) -> str:
    ...
    llm_notice_required = truthy(metadata.get("disclaimer_required"))
    ...
    if language_mode == "en":
        ...
        llm_notice = (
            "This report includes initial guidance generated by a large language model (LLM); it may be incomplete "
            "or incorrect and must be used as orientation, not as a diagnosis. "
        )
        disclaimer = (llm_notice if llm_notice_required else "") + base_disclaimer
        ...
        llm_notice = (
            "Este reporte incluye inferencias iniciales generadas por un modelo de lenguaje (LLM); pueden ser "
            "incompletas o incorrectas y deben usarse como orientacion, no como diagnostico. "
        )
        disclaimer = (llm_notice if llm_notice_required else "") + base_disclaimer
