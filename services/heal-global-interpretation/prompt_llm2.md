...
2. A deterministic backend summary with grouped information by gene, rsID, biological module/category, confidence level, repeated rsIDs, multiple variants in the same gene, and review flags.
...
5. Do not assume that all variants in the same broad category interact directly.
...
2. Identify the strongest biological axes or systems suggested by the data.
...
- If different genes fall within the same functional axis, you may discuss that as a broader pathway-level pattern only when supported by the deterministic summary and individual interpretations.
