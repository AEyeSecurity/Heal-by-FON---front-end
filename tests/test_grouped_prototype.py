...
class GroupedPrototypeTests(unittest.TestCase):
    ...
    def test_runtime_allowlist_collects_all_explicit_payload_evidence(self):
        payload = {
            "focus_variant_evidence": [{"evidence_id": "evv_1"}],
            "mechanism_evidence": {"evidence_id": "evm_1"},
            "knowledge_context": [{"evidence_id": "evk_1"}],
            "gwas_evidence_summary": {"prioritized_clusters": [{"cluster_ref": "gwc_1"}]},
        }
        self.assertEqual(MODULE.runtime_evidence_ids(payload), {"evv_1", "evm_1", "evk_1", "gwc_1"})
    ...
    def test_dry_run_produces_both_reports_and_preserves_registry(self):
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertEqual(sum(row["coverage_status"] == "covered_by_prototype_snapshot" for row in cards), 12)
